...
import { getLoanWithDetails } from "./loan.service";
...
//     if (!loan.amount) {
//         throw new Error("Loan not created yet");
//       }
...
// }
//update
// services/underwriting.service.ts
// export async function processLoanUnderwriting(
//     loanId: number
//   ): Promise<{
//     approved: boolean;
//     foir: number;
...
//     const loan = await prisma.loan.findUnique({
//       where: { id: loanId },
//       include: { user: true, verificationResults: true },
//     });
  
//     if (!loan) throw new Error("Loan not found");
//     if (!loan.amount) throw new Error("Loan amount missing");
//     if (!loan.tenure_months) throw new Error("Loan tenure missing");
//     if (!loan.monthlyincome) throw new Error("Monthly income missing");
...
//     const result = runUnderwritingLogic({
//       applicationId: loan.id.toString(),
...
//updated function by me to make things work
...
export async function processLoanUnderwriting(loanId: number) {
    ...
    if(!loan) throw new Error ("Loan is nulll");
    if (
        loan.amount == null ||
        loan.monthlyincome == null ||
        loan.tenure_months == null
      ) {
        throw new Error("Insufficient data for underwriting");
      }
    const annualRate = 12;
    const salary = loan.monthlyincome;
    ...
    const foir = emi / salary;
    if (foir > 0.5) {
      return {
        approved: false,
        rejectionReason: "FOIR exceeds allowed limit",
      };
    }
